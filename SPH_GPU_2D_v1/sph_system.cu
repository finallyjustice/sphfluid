...
__device__ uint calc_grid_hash(int2 grid_pos)
{
...
void calc_hashK(uint *dHash, uint *dIndex, Particle *dMem, uint num_particle)
{
	...
	int2 grid_pos=calc_grid_pos(dMem[index].pos);
							...
							uint tot_cell)
{
	...
	int2 cell_pos=calc_grid_pos(dMem[index].pos);
...
__device__
float2 compute_cell_force(uint index, int2 neighbor, Particle *dMem, uint *dHash, uint *dIndex, uint *dStart, uint *dEnd, uint num_particle, uint tot_cell, float2 &grad_color, float &lplc_color)
{
